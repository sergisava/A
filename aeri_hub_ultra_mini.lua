...
local script_content = _HttpGet and _HttpGet(game, SCRIPT_URL, true)
if not script_content or #script_content < 1000 then
    if HttpService and _RequestAsync then
        local r = _RequestAsync(HttpService, {Url = SCRIPT_URL, Method = "GET"})
        if r and r.Success and r.Body then
            script_content = r.Body
        end
    end
end

if not script_content or #script_content < 1000 then
    warn("[AERI] Error descargando script")
    return
end
